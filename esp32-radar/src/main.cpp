 ...
 * @brief 固件程序入口和系统启动流程。
 *
 * 初始化串口、Wi-Fi、雷达、红外、BLE 及全部 FreeRTOS 任务。
 */
#include <Arduino.h>
#include <WiFi.h>
...
void setup() {
    ...
    // UART0改为仅发送：保留GPIO43(TX0)日志，释放GPIO44(RX0)给PIR。
    Serial.flush();
    Serial.end();
    delay(20);
    Serial.begin(115200, SERIAL_8N1, -1, 43);
    initPirManager();                       // 初始化HC-SR501红外输入GPIO44
